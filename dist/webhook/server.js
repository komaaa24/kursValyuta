...
exports.startWebhookServer = void 0;
...
const finalizePayment = async (payment, metadata, deps, telegramIdOverride) => {
    ...
    if (!payment.appliedAt) {
        ...
        const updatedUntil = await (0, proService_1.applyProPayment)(deps.userRepository, user, deps.proDurationDays, paidAt);
        payment.appliedAt = new Date();
        await deps.paymentRepository.save(payment);
        try {
            const untilLabel = deps.proDurationDays <= 0 ? "cheksiz" : formatDate(updatedUntil);
            await deps.bot.api.sendMessage(telegramId, ["✅ PRO faollashtirildi!", `Muddati: ${untilLabel}`, "Botga qaytib foydalanishingiz mumkin."].join("\n"));
        }
        catch (error) {
...
const handleClickPrepare = async (body, res, deps) => {
    ...
    const secretKey = env_1.env.click.secretKey ?? "";
    const signatureValid = (0, clickService_1.verifyClickSignature)(clickTransId, serviceId, secretKey, merchantTransId, amount, action, signTime, signString);
    if (!signatureValid) {
        sendJson(res, 200, {
    ...
    await deps.paymentRepository.save(payment);
    const responseSignature = (0, clickService_1.generateClickResponseSignature)(clickTransId, serviceId, secretKey, merchantTransId, String(payment.id), amount, action, signTime);
    sendJson(res, 200, {
        click_trans_id: clickTransId,
        merchant_trans_id: merchantTransId,
        merchant_prepare_id: payment.id,
...
const handleClickComplete = async (body, res, deps) => {
    const clickTransId = coerceString(body.click_trans_id);
    const serviceId = coerceString(body.service_id);
    const merchantTransId = coerceString(body.merchant_trans_id);
    ...
    const secretKey = env_1.env.click.secretKey ?? "";
    const signatureValid = (0, clickService_1.verifyClickSignature)(clickTransId, serviceId, secretKey, merchantTransId, amount, action, signTime, signString, merchantPrepareIdRaw);
    if (!signatureValid) {
        sendJson(res, 200, {
            click_trans_id: clickTransId,
            merchant_trans_id: merchantTransId,
    ...
    if (payment.status === Payment_1.PaymentStatus.PAID) {
        const responseSignature = (0, clickService_1.generateClickResponseSignature)(clickTransId, serviceId, secretKey, merchantTransId, merchantPrepareIdRaw || String(payment.id), amount, action, signTime);
        sendJson(res, 200, {
            click_trans_id: clickTransId,
            merchant_trans_id: merchantTransId,
...
const handleClickWebhook = async (req, res, deps) => {
