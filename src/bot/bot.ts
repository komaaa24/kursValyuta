import { Bot, Context, InlineKeyboard, Keyboard, NextFunction } from "grammy";
import fetch from "node-fetch";
import { Repository } from "typeorm";
import { env } from "../config/env";
import { Payment, PaymentStatus } from "../entities/Payment";
import { RateAlert, AlertDirection } from "../entities/RateAlert";
...
type ProPaymentDetails = {
  url: string;
  tx: string;
};

type ProPaymentResult =
  | { ok: true; details: ProPaymentDetails }
...
const convertPairKeyboard = (): InlineKeyboard =>
  new InlineKeyboard()
...
const ratesKeyboard = (): InlineKeyboard =>
  new InlineKeyboard()
    .text("⬅️ Orqaga", "menu:open:home");

const alertsKeyboard = (): InlineKeyboard =>
  new InlineKeyboard()
...
const helpKeyboard = (): InlineKeyboard =>
  new InlineKeyboard()
...
const commandKeyboard = new Keyboard()
  .text("💱 Konvertatsiya")
  .text("📊 Kurslar")
  .row()
  .text("⭐ Pro")
...
const isMessageNotModifiedError = (error: unknown): boolean => {
  ...
  const maybe = error as { description?: unknown; message?: unknown };
...
const isChatUnavailableError = (error: unknown): boolean => {
  if (!error || typeof error !== "object") return false;
  const maybe = error as { description?: unknown; message?: unknown; error_code?: unknown };
  const text = typeof maybe.description === "string" ? maybe.description : typeof maybe.message === "string" ? maybe.message : "";
...
export const createBot = (token: string, deps: Dependencies): Bot<Context> => {
  ...
  const alertCheckIntervalMs = env.alertCheckIntervalMs;
  const proPrice = Number.isFinite(env.proPrice) && env.proPrice > 0 ? env.proPrice : 1111;
  const proDurationDays = Number.isFinite(env.proDurationDays) ? env.proDurationDays : 30;
  ...
  const ensureUser = async (ctx: Context): Promise<User | null> => {
  ...
  const isProUser = (user: User | null): boolean => Boolean(user?.proUntil && user.proUntil.getTime() > Date.now());

  const formatSom = (value: number): string => `${value.toLocaleString("en-US")} so'm`;

  const proPayLabel = (price: number): string => `💳 Click orqali to'lov (${formatSom(price)})`;

  const isAdmin = (telegramId?: number): boolean => Boolean(telegramId && adminIds.includes(telegramId));
  ...
  const buildProKeyboard = (payment?: ProPaymentDetails): InlineKeyboard => {
  ...
  const buildProOfferText = (): string =>
    [
  ...
  const createProPayment = async (ctx: Context): Promise<ProPaymentResult> => {
    const userId = ctx.from?.id;
    if (!userId) {
      return { ok: false, message: "Foydalanuvchi aniqlanmadi. Qaytadan urinib ko'ring." };
    }

    if (!env.click.enabled && !env.proPaymentUrl) {
  ...
  const handleProPaymentStart = async (ctx: Context): Promise<void> => {
  ...
  const handleProPaymentCheck = async (ctx: Context, transactionParam?: string): Promise<void> => {
    await ctx.answerCallbackQuery();
    const userId = ctx.from?.id;
    if (!userId) {
    ...
    const paidAtRaw = payment.metadata && typeof payment.metadata === "object" ? (payment.metadata as Record<string, unknown>).paidAt : undefined;
    const paidAt = typeof paidAtRaw === "string" ? new Date(paidAtRaw) : new Date();
    const updatedUntil = await applyProPayment(deps.userRepository, user, proDurationDays, paidAt);
  ...
  const sendProOffer = async (ctx: Context, opts?: { edit?: boolean }) => {
    const text = buildProOfferText();
  ...
  const ensureProAccess = async (ctx: Context, opts?: { edit?: boolean }): Promise<boolean> => {
  ...
  const handleProEntry = async (ctx: Context, opts?: { edit?: boolean }): Promise<void> => {
    const user = await getUserByTelegramId(ctx.from?.id);
    if (isProUser(user)) {
  ...
  const parseAlertTarget = (text: string): { target: number; direction: AlertDirection } | null => {
  ...
  const sendRatesList = async (ctx: Context, opts?: { edit?: boolean; offset?: number }) => {
    ...
    const keyboard =
      nextOffset < rates.length ? new InlineKeyboard().text("▶️ Davomi", `rates:more:${nextOffset}`).row().text("⬅️ Orqaga", "menu:open:home") : undefined;
  ...
  const fetchProBankRates = async (): Promise<BankRatesPayload | null> => {
    try {
      const response = await fetch(env.proBanksUrl);
      if (!response.ok) {
        console.error(`Pro bank rates request failed (${response.status} ${response.statusText})`);
        return null;
      }
      const payload = (await response.json()) as BankRatesPayload;
  ...
  const sendProBankRates = async (ctx: Context, kind: BankRateKind, opts?: { edit?: boolean; offset?: number }) => {
    ...
    const keyboard = new InlineKeyboard().text("🏦 Sotib olish", "pro:banks:buy:0").text("🏦 Sotish", "pro:banks:sell:0").row();
    if (nextOffset < total) {
  ...
  const runAlertCheck = async (): Promise<void> => {
    ...
    try {
      ...
      const getUserCached = async (telegramId: number): Promise<User | null> => {
        if (userCache.has(telegramId)) return userCache.get(telegramId) ?? null;
  ...
  bot.command("start", async (ctx) => {
    ...
    if (ctx.from) userState.delete(ctx.from.id);
  ...
  bot.on("callback_query:data", async (ctx) => {
    ...
    switch (module) {
      ...
      case "pro": {
        ...
        if (action === "banks") {
          const kind: BankRateKind = value === "sell" ? "sell" : "buy";
...
const chunkMessage = (message: string, limit = 3800): string[] => {
...
const flagFor = (code: string): string => CURRENCY_FLAGS[code] ?? "🏳️";
...
const formatAmount = (value: number): string =>
  Number.isFinite(value) ? value.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : String(value);

const formatBankRate = (entry: BankRateEntry): string => {
...
const normalizeProBankDate = (value?: string): string | null => {
...
const parseNormalizedDateParts = (value: string): { day: number; month: number; year: number } | null => {
...
const formatUzbekMonthDay = (day: number, month: number): string => {
  const monthName = UZBEK_MONTHS[month - 1];
  if (!monthName) return String(day);
  return `${day} - ${monthName}`;
};

const resolveProBankDateLabel = (payload: BankRatesPayload): { label: string; fromPayload: boolean } => {
  const normalized =
    normalizeProBankDate(payload.source) ??
...
const formatProBankTitle = (title: string, payload: BankRatesPayload): string => {
  ...
  if (monthPattern.test(title)) return title;
  return `${title} (${dateLabel})`;
};

const parseUsdInlineAmount = (text: string): number | null => {
  if (!text || text.trim().startsWith("/")) return null;
  const trimmed = text.trim();
  if (/^(usd|\$)$/i.test(trimmed)) return 1;
...
const parseFreeAmount = (text: string): number | null => {
  if (!text || text.trim().startsWith("/")) return null;
...
const trendEmoji = (value: number): string => {
  if (value > 0) return "📈";
  if (value < 0) return "📉";
  return "⏸️";
};
...
const proText = (price: number): string =>
  [
